...
public:
    ...
    virtual Color3f Li(const Scene *scene, Sampler *sampler, const Ray3f &_ray) const override {
        ...
        for (int i = 0; i < m_iterCount; i ++) {
            ...
            /* post-process: adjust illumination */
            for (auto ht : m_hitpoints) {
                std::vector<uint32_t> results;
                m_photonMap->search(ht.x, ht.R, results);
                float density = (ht.N + results.size()) * INV_PI / ht.R;

                /* radius reduction */
                int nextN = (int) ht.N + m_alpha * results.size();
                float dR = ht.R - ht.R * sqrt(nextN / (ht.N + results.size()));
                ...
                nextFlux = (ht.tao + nextFlux) * nextN / (ht.N + results.size());
            }

            m_photonMap->clear();
        }

        /// TODO: need to change how rays are sampled
        /// TODO: OR I do the photon passes for each ray sample separately
        /* radiance evaluation */
